...
def main():
    parser = argparse.ArgumentParser(
        description="Plot max_abs_dx_deformation from pair_offset_summary.csv."
    )
    parser.add_argument(
        "--summary",
        default="output_pair_offset/pair_offset_summary.csv",
        help="Path to pair_offset_summary.csv.",
    )
    ...
    with open(args.summary, newline="") as f:
        ...
        for row in reader:
            ...
            if args.start <= time <= args.end:
                times.append(time)
                values.append(float(row["max_abs_dx_deformation"]))
                max_i.append(int(row["max_def_i"]))
                max_j.append(int(row["max_def_j"]))
    ...
    plt.xlabel("time [s]")
    plt.ylabel("max_dx_deformation [mm]")
    ...
    #     f"max_abs_dx_deformation={values[peak_idx]:.12g}, "
    #     f"(i,j)=({max_i[peak_idx]},{max_j[peak_idx]})"
    # )
